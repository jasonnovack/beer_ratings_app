...
const getTalliesTemplate = require('./frontend/templates/get-tallies-template');
...
const authCheck = async(req, res) => {
  let valid = await pwdb.getPassword(req);
  let allow = false;
  if (valid) {
    allow = true;
  }
...
const postBeer = async (req, res) => {
  ...
  if (authorized) {
    ...
    try {
      let response = {};
      response.postBeer = await db.postBeer(req);
      if (response.postBeer) {
        response.incrementTallies = await tallydb.increment(req, item.score);  
      }
      res.send(response);
    } catch (e) {
      console.error(e);
...
const getTallies = async (req, res) => {
  try {
    let response = await tallydb.listTallies(req);
    console.log('RESPONSE-----', response);
    const html = await getTalliesTemplate(response);
    res.set('Content-Type', 'text/html');
    res.send(html);  
  } catch (e) {
    console.error(e);
...
const postTally = async (req, res) => {
  let score = req.body.score;
  let count = req.body.count;
  try {
    let response = await tallydb.putTally(req, score, count);
    res.send(response);
  } catch (e) {
    console.error(e);
    res.done(e, null);
